...
class AuthApi {
  ...
  static Future<Result<String>> register({
    ...
    try {
      // Поскольку API не поддерживает POST для регистрации, 
      // попробуем использовать GET с параметрами
      final url = Uri.parse('${AppConfig.apiBaseUrl}/user/register').replace(
        queryParameters: {
          'name': name,
          'email': email,
          'password': password,
      ...
      print('[AUTH_API] register: GET $url');
      final res = await http.get(url, headers: headers);

      print('[AUTH_API] register: Status ${res.statusCode}');
      print('[AUTH_API] register: Response: ${res.body}');
      ...
      if (res.statusCode == 200 || res.statusCode == 201) {
        ...
        if (data is Map<String, dynamic>) {
          ...
          if (token != null && token.isNotEmpty) {
            ...
            // Если токен не найден, возможно регистрация прошла успешно, но нужно залогиниться
            print('[AUTH_API] register: Trying to login after registration...');
            return await login(email, password);
          }
        } else {
          print('[AUTH_API] register: Unexpected response format: $data');
          return const Err('Неожиданный формат ответа');
        }
      } else {
        final data = jsonDecode(res.body);
  ...
  static Future<Result<String>> login(String email, String password) async {
    try {
      ...
      print('[AUTH_API] login: Response: ${res.body}');
        ...
        if (data is Map<String, dynamic>) {
          if (data['message'] is String) {
            ...
            if (dataMap['form'] is List && (dataMap['form'] as List).isNotEmpty) {
              message = (dataMap['form'] as List).first.toString();
            }
          }
        }
