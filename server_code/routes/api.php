...
/*
 * SECURITY: Dangerous routes disabled in production.
 * - /clear-cache: Use admin panel or AdminsController::clearCache (auth:admin).
 * - Install/migration/update-env: Enable only during initial install, then disable.
 * - /nefedfsrgw: Contained migrate:fresh — REMOVED (could wipe DB).
 * - deactivate: Use only via admin auth group (see prefix admin).
 */
if (env('APP_ENV') === 'local' && env('ALLOW_INSTALL_ROUTES', false)) {
...
Route::get('/nefedfsrgw', function (\Illuminate\Http\Request $request) {
    // DISABLED: This route previously ran migrate:fresh and could wipe the database.
    abort(404);
});
