...
class SubscriptionEmailsController extends ControllerHelper
{
    ...
    public function sendSubscriptionEmail(Request $request)
    {
        try {
            ...
            // Отправляем push-уведомления всем пользователям с FCM токенами
            $pushStats = null;
            try {
                $pushService = app(\App\Services\PushNotificationService::class);
                $pushStats = $pushService->sendPromotionNotification(
                    $subscriptionEmailFormat->subject,
                    $subscriptionEmailFormat->body
                );

                \Log::info('Subscription email: Push notifications sent', [
                    'email_subject' => $subscriptionEmailFormat->subject,
                    'push_stats' => $pushStats
                ]);
            } catch (\Exception $pushEx) {
                // Логируем ошибку пуша, но НЕ прерываем ответ, чтобы email рассылка прошла
                \Log::error('Subscription email: Push notification failed', [
